    ...
    if not trajectory_exists:
        ...
        if trajectory.aux["log"]["terminate"] == "":
            ...
            if advanced_settings["enable_mpnn"]:
                ...
                if mpnn_sequences:
                    ...
                    for mpnn_sequence in mpnn_sequences:
                        ...
                        if not pass_af2_filters:
                            logger.info(f"Base AF2 filters not passed for {mpnn_design_name}, skipping interface scoring")
                            mpnn_n += 1
                            continue
                    ...
                    if accepted_mpnn >= 1:
                        logger.info("Found "+str(accepted_mpnn)+" MPNN designs passing filters")
                        logger.info("")
                    else:
                    ...
                    logger.info('Duplicate MPNN designs sampled with different trajectory, skipping current trajectory optimisation')
                    logger.info("")
                ...
                logger.info("Design and validation of trajectory "+design_name+" took: "+design_time_text)
            ...
            if trajectory_n >= advanced_settings["start_monitoring"] and advanced_settings["enable_rejection_check"]:
                ...
                if not acceptance >= advanced_settings["acceptance_rate"]:
                    logger.info("The ratio of successful designs is lower than defined acceptance rate! Consider changing your design settings!")
...
logger.info("Finished all designs. Script execution for "+str(trajectory_n)+" trajectories took: "+elapsed_text)